...
function atualizaAnimacao(){
    ...
    // Limites da tela para as barras não sumirem
    ...
    // Avalia se o X da bola tocou a borda da barra e se o Y da bola está na altura da barra
    if (txBola - 0.05 <= -0.85 && txBola + 0.05 >= -0.95) {
        if (tyBola >= tyBE - 0.25 && tyBola <= tyBE + 0.25) {
            txBola_offset = Math.abs(txBola_offset); // Rebate pra direita
        }
    }

    // Colisão com a Barra Direita
    if (txBola + 0.05 >= 0.85 && txBola - 0.05 <= 0.95) {
        if (tyBola >= tyBD - 0.25 && tyBola <= tyBD + 0.25) {
            txBola_offset = -Math.abs(txBola_offset); // Rebate pra esquerda
        }
    }

    // Rebate nas paredes do fundo (Caso a bola passe da barra, para o jogo não quebrar)
